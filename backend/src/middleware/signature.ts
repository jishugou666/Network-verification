...
export async function signatureMiddleware(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  // Nonce 防重放：检查 5 分钟内是否已使用
  const nonceExpiry = new Date(Date.now() - config.nonceTtlSeconds * 1000);
  const existingNonce = await prisma.nonce.findFirst({
    where: { appKey, nonce, createdAt: { gte: nonceExpiry } },
  });
  if (existingNonce) {
