...
import { generateCardKey, aesEncrypt, aesDecrypt, hmacSign } from '../utils/crypto';
...
export async function generateCards(agentId: string, role: string, data: GenerateCardsRequest) {
  ...
  // 解密 AppSecret 用于计算 HMAC 索引
  let appSecret: string;
  try {
    appSecret = aesDecrypt(program.appSecretEncrypted);
  } catch {
    return { code: 500, message: '程序密钥解密失败', data: null };
  }
  ...
  // 批量生成卡密
  const generatedCards: { plain: string; hash: string; hmacIndex: string }[] = [];
  for (let i = 0; i < data.count; i++) {
    const plain = generateCardKey(prefix);
    const hash = await bcrypt.hash(plain, BCRYPT_ROUNDS);
    const hmacIndex = hmacSign(plain, appSecret);
    generatedCards.push({ plain, hash, hmacIndex });
  }
