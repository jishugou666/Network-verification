...
// 使用内存缓存 + TiDB 持久化，适配 Serverless 无状态环境
// 内存缓存提供快速拦截，数据库保证多实例部署时一致性
...
function cleanupMemCache(): void {
  const now = Date.now();
  if (now - lastMemCleanup < 60000) return;
  lastMemCleanup = now;
  for (const [key, record] of memCache) {
    if (now - record.windowStart.getTime() > 120000) {
      memCache.delete(key);
    }
  }
}
